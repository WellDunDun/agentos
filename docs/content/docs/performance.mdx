...
- Each benchmark **creates the sidecar once** (`AgentOs.createSidecar()`) and
  ...
  figures measure the marginal cost of a same-trust-domain VM, not a fresh
  process. `AgentOs.create()` with no `sidecar` option deliberately uses a fresh
  process per VM; benchmark code opts into sharing to measure the lower-level
  incremental runtime cost.
