...
- Every VM created with `AgentOs.create()` gets a **dedicated sidecar OS
  process** by default. Disposing the VM also destroys that process.
- This process boundary prevents mutually untrusted V8 isolates from sharing an
  address space. A V8 isolate alone is not a Spectre security boundary.
- Automatic for `agentOS()`, `AgentOs.create()`, and Rivet Actors.
- Advanced: the direct VM API exposes explicit shared pools and sidecar handles
  for lower startup and memory overhead. Treat every VM on one shared handle as
  the **same trust domain**; do not mix mutually untrusted tenants in it.
- See [Performance](/agentos/docs/performance) for benchmarks that explicitly
  opt into shared placement when measuring marginal VM cost.
