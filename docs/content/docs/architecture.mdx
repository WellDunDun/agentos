...
The client speaks to the agentOS server over the wire. The server runs the **sidecar**, the trusted core that hosts a VM: it owns the VM's kernel and brokers every guest syscall the agent makes (filesystem, processes, network, permissions) before carrying it out. Direct `AgentOs.create()` calls use one sidecar OS process per VM by default. Explicit shared placement can host multiple same-trust-domain VMs in one process.
...
- **Hosts one trust domain.** Default placement gives each VM a dedicated sidecar process. Explicit shared placement can manage many VMs side by side for performance, but all V8 isolates in that process must belong to the same trust domain.
...
- **The unit of logical isolation.** Put one tenant or one task per VM to control the blast radius; keep the default dedicated sidecar placement when tenants do not trust one another.
