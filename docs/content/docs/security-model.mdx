...
- **Sandboxed execution.** All agent code runs inside a V8 isolate with WebAssembly. Mutually untrusted VMs also get separate sidecar OS processes by default, because an isolate alone is not a speculative-execution boundary.
...
- **Per-actor containment.** Each actor is its own VM and, by default, its own sidecar process. Two actors share no filesystem, globals, module state, memory, address space, or crash fate. Explicit shared-sidecar placement is a same-trust-domain performance optimization, not a boundary between mutually untrusted tenants.
...
- **Filesystem.** A virtual, in-memory filesystem. Guest reads and writes never reach the real host filesystem. Host data enters the VM only through the `files`, `mounts`, or `nodeModules` you configure explicitly. See [Filesystem](/agentos/docs/filesystem).
- **Processes.** `node:child_process` spawns kernel-managed guest processes, never real host processes. Children can only run the commands the VM mounts (WASM-backed `sh` and coreutils, V8-backed `node`). See [Processes](/agentos/docs/processes).
- **Network.** Guest `fetch()`, `node:http`, and raw sockets all flow through the kernel socket table. Guest `fetch()` runs through undici inside the isolate and then through the kernel socket table; it never opens a real host socket. See [Networking](/agentos/docs/networking).
- **DNS, pipes, and PTYs** are likewise kernel-owned: no guest path reaches the host directly.
- **Bindings.** Registered [bindings](/agentos/docs/bindings) are the only sanctioned way to hand the guest a named host capability. The guest invokes a binding by name with JSON input, the call round-trips to the host handler, and only the handler's return value comes back. The guest never receives the underlying host access.
...
A scope can be `"allow"`, `"deny"`, or a `{ default, rules }` policy that matches request patterns. Guest servers are reachable only over loopback inside the VM unless you exempt a port explicitly. See [Permissions](/agentos/docs/permissions) and [Networking](/agentos/docs/networking) for the full policy shape.
...
- **Timing defenses in depth.** In the default mode, high-resolution clocks (`Date.now()`, `performance.now()`, `process.hrtime()`) are frozen within a run and `SharedArrayBuffer` is removed. Those controls raise the cost of timing attacks, but remote I/O can still act as a timer; the default cross-tenant boundary is the sidecar OS process, not timer coarsening or a V8 isolate.

See [Security & Auth](/agentos/docs/security-model) for resource limits, network control, and authentication setup.
...
- With default placement, a crash, resource exhaustion, or escape attempt is contained to the VM's dedicated sidecar process.
...
- Putting only same-trust-domain VMs on an explicit shared sidecar handle or pool.
...
- [Security configuration](/agentos/docs/security-model) for resource limits, network control, and authentication setup
- [Permissions](/agentos/docs/permissions) for agent tool-use approval patterns
- [agentOS vs Sandbox](/agentos/docs/versus-sandbox) for when to escalate to a full sandbox
