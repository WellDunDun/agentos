...
Network permission checks run before [outbound HTTP middleware](/docs/outbound-http-middleware) is invoked. A denied guest request fails with `EACCES` and cannot cause trusted host middleware to run.
