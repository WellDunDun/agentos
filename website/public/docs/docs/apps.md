...
for (let attempt = 0; attempt < 3; attempt++) {
  try {
    ...
    if (error instanceof AgentOSAppsError && error.serverFault) throw error;
...
Host/platform failures use `agentos_apps_server_fault`, set `serverFault: true`,
and include a bounded, redacted error chain in `metadata.chain`; changing app
source cannot repair those failures.
...
Rivet namespace list and create permissions. `warmTimeoutMs` is the bounded
per-replica startup deadline and must be from `1000` through `600000`.
