...
Guest JavaScript HTTP clients have one additional pre-connect interception point. When [outbound HTTP middleware](/docs/outbound-http-middleware) is configured, the sidecar canonicalizes the logical URL authority, checks `network.http` permission, selects the exact-host or catch-all route, and invokes the VM-scoped host callback. A selected route never opens an upstream socket; an unmatched request continues into the ordinary kernel TCP/TLS path. HTTPS is selected from the semantic request before TLS starts, so this implementation does not terminate or re-sign guest TLS.
