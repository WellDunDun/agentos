...
Limits are observable, not just enforced. Call `agentOs.getSystemInfo()` to get
the sidecar-owned catalog for a VM. The response includes every effective limit,
its configuration path and description, whether it came from an explicit VM
configuration or the sidecar default, and live usage where the runtime owns a
VM-scoped gauge. The agentOS inspector renders this same response in its
**Limits** tab, ordered by percent consumed.

Measured entries include current usage, capacity, fill percent, and a high-water
mark for the lifetime of that VM. Configuration-only limits such as deadlines
and per-operation payload caps remain in the catalog with their effective value,
but report no live usage or high-water mark rather than inventing a gauge.
