...
// The CLI command path (`agentos-<kit> <binding>`) is gated by `bindingPermissionMode`
...
// branch (`handleHostCommandCallback` -> `handleAgentOsBindingGroupCommand` ->
...
const duplicateMathBindingGroup = bindingGroup({
	name: "math",
...
describe("bindingGroup permissions", () => {
	...
	test("rejects duplicate binding registration with a conflict", async () => {
		await expect(
			AgentOs.create({
				bindings: [mathBindingGroup, duplicateMathBindingGroup],
			}),
		).rejects.toThrow(/conflict: bindingGroup already registered: math/);
	});

	test("allows bindingGroup invocation with default permissions", async () => {
		vm = await AgentOs.create({
			software: [common],
			bindings: [mathBindingGroup],
		});
	...
	test("denies bindingGroup invocation by default until binding permissions are granted", async () => {
		vm = await AgentOs.create({
			software: [common],
			bindings: [mathBindingGroup],
			permissions: {
	...
	test("allows bindingGroup invocation when a matching binding permission is granted", async () => {
		vm = await AgentOs.create({
			software: [common],
			bindings: [mathBindingGroup],
			permissions: {
...
describe("bindingGroup permissions — raw host_callback RPC path", () => {
	let vm: AgentOs | null = null;
