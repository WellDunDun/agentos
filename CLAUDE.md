...
- Registry software build philosophy — ship unmodified upstream software, not WASM ports. This is owned and enforced in secure-exec's `registry/native` (paths below are in the secure-exec dependency); agent-os only consumes the built `@agentos-software/*` WASM packages, but the rule binds anyone touching that build.
  - The goal is to run **unmodified upstream Linux software** in the VM, compiled to `wasm32-wasip1` against our **custom (patched) libc + Rust std** — NOT to write WASM-specific reimplementations of each tool. All portability work lives in the libc/std layer, never in a fork of the program.
  - How it works (`registry/native/`): upstream crates (uutils coreutils, ripgrep/grep, fd, findutils, gawk, jq, …) build unmodified via `-Z build-std` against a Rust `wasm32-wasip1` sysroot patched by `registry/native/patches/*.patch` (applied by `scripts/patch-std.sh`). Those patches add WASI process spawn, pipes, fd dup, user/group, isatty, temp dir, and file mode through custom `host_process` / `host_user` WASM imports — i.e. we extend the standard library, not the application.
  - When a program fails to build or misbehaves, fix it in the patched std/libc layer (a new patch under `registry/native/patches/`) or the host import surface — never by adding `cfg(target_*)` conditionals, shims, or behavioral forks inside the software's own source. A WASM-specific branch in application code is a sign the fix belongs one layer down.
  - `registry/native/crates/commands/*` are thin standalone-binary entrypoints over upstream library crates (`registry/native/crates/libs/*`); keep them as adapters, not reimplementations.
