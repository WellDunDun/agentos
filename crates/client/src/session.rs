...
use crate::config::{AgentOsConfig, BindingGroup, MountConfig};
...
fn build_host_binding_reference(bindings: &[BindingGroup]) -> String {
    ...
    for group in bindings {
        ...
        for binding in &group.bindings {
            ...
            lines.push(format!(
                "- `agentos-{} {}{}` - {}",
                group.name, binding.name, suffix, binding.description
            ));
        }
        lines.push(String::new());
        lines.push(format!(
...
impl AgentOs {
    ...
    pub async fn create_session(
        ...
        let binding_reference = build_host_binding_reference(&self.config().bindings);
