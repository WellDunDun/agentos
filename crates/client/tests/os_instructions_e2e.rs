...
//! the Agent OS client passes create-time additions and generated binding docs to the wrapper sidecar,
...
use agentos_client::config::{
    AgentOsConfig, AgentOsSidecarConfig, Binding, BindingGroup, FsPermissions, PatternPermissions,
...
async fn launch_pi_session_and_read_argv(options: CreateSessionOptions) -> Vec<String> {
    launch_pi_session_with_bindings_and_read_argv(options, Vec::new()).await
}

async fn launch_pi_session_with_bindings_and_read_argv(
...
async fn create_session_injects_host_tool_reference_from_client_config() {
    ...
    let argv = launch_pi_session_with_bindings_and_read_argv(
        CreateSessionOptions::default(),
        vec![BindingGroup {
            name: "weather".to_string(),
            description: "Weather lookup bindings.".to_string(),
    ...
    assert!(
        ...
        "client-generated binding reference is injected: {prompt:?}"
    );
    assert!(
        prompt.contains("`agentos-weather forecast --zip-code <string>`"),
        "binding reference includes CLI command and schema-derived flags: {prompt:?}"
    );
}
